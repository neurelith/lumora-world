...
import { RuneRealmResult } from '@/lib/types';
import { Card } from '@/components/ui/Card';
import { TriageIndicator } from '@/components/ui/TriageIndicator';
import { Button } from '@/components/ui/Button';

export default function RuneRealmDemoPage() {
  const [result, setResult] = useState<RuneRealmResult | null>(null);
      ...
      <div>
        <header className="max-w-4xl mx-auto flex items-center justify-between pb-6">
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs font-display font-bold uppercase text-realm bg-realm-light/10 px-3 py-1 rounded-full border border-realm/20 flex items-center gap-1.5">
              <Wand2 className="w-3.5 h-3.5 text-amber-600" />
              <span>Camera Air-Tracing + Touch · Rune Realm</span>
            </span>
          </div>
        </header>
        ...
        <main id="main-content" className="max-w-4xl mx-auto">
          {!result ? (
            <RuneRealm grade={2} language="en" onComplete={(res) => setResult(res)} />
          ) : (
            <Card className="p-8 text-center space-y-6 max-w-lg mx-auto bg-white border-2 border-realm">
              <h2 className="text-2xl font-bold font-display text-ink">Rune Realm Demo Complete!</h2>
              <div className="space-y-2 text-sm font-body">
                <p><strong>NVI (Velocity Inversions):</strong> {result.meanNvi}</p>
                <p><strong>Jerk Index (Smoothness):</strong> {result.meanJerkIndex}</p>
                <p><strong>Mirror Reversals:</strong> {result.mirrorReversalsCount}</p>
              </div>

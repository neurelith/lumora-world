...
export default function HavenPage() {
  ...
  return (
    <div className={`min-h-screen bg-world-haven dark:bg-[#0B0F19] text-ink dark:text-slate-100 flex flex-col p-4 sm:p-6 md:p-8 transition-colors duration-200 ${sensoryCalm ? 'sensory-calm' : ''}`}>
      {/* Top Header Bar */}
      <header className="max-w-4xl mx-auto w-full flex items-center justify-between gap-3 pb-5 border-b-2 border-amber-200 dark:border-slate-800">
        <Link
          href="/"
          className="flex items-center gap-2 text-sm font-display font-extrabold text-ink/70 dark:text-slate-300 hover:text-ink dark:hover:text-white transition-colors bg-white/80 dark:bg-slate-800/80 px-3 py-1.5 rounded-full border border-hairline dark:border-slate-700 shadow-soft-xs"
        >
        ...
        {/* Action buttons: Calm Mode Toggle */}
        <div className="flex items-center gap-2">
          <button
            onClick={() => setSensoryCalm(!sensoryCalm)}
            aria-pressed={sensoryCalm}
            title="Toggle Sensory Calm Mode"
            className={`min-h-[40px] px-3.5 rounded-2xl border-2 flex items-center gap-2 font-display text-xs font-extrabold transition-all ${
              sensoryCalm
